...
export default function LoginToggleModal({ user, onConfirm, onCancel }) {
  if (!user) return null;

  const isEnabling = !user.loginEnabled;
      ...
      <div className="fixed left-1/2 top-1/2 z-[9999] -translate-x-1/2 -translate-y-1/2 w-full max-w-sm mx-4 animate-slide-in">
        <div className="relative rounded-lg border border-[var(--semantic-border-layout)] bg-[var(--semantic-bg-primary)] shadow-lg p-6">
          ...
          <h2 className="text-lg font-semibold text-[var(--semantic-text-primary)] text-center">
            {isEnabling ? 'Enable Login Access' : 'Disable Login Access'}
          </h2>

          {/* Description */}
          <p className="mt-3 text-sm text-[var(--semantic-text-secondary)] text-center leading-relaxed">
            Are you sure you want to {isEnabling ? 'enable' : 'disable'} login for {user.name}?
          </p>
          ...
          <div className="mt-6 flex justify-center gap-3">
            <button
              onClick={onConfirm}
              className={`inline-flex items-center justify-center gap-2 whitespace-nowrap min-w-[100px] h-10 px-5 rounded text-sm font-medium text-[var(--semantic-text-inverted)] transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 ${
                isEnabling
                  ? 'bg-[var(--semantic-primary)] hover:bg-[var(--semantic-primary-hover)] focus-visible:ring-[var(--semantic-primary)]'
                  : 'bg-[var(--semantic-error-primary)] hover:bg-[var(--semantic-error-hover)] focus-visible:ring-[var(--semantic-error-primary)]'
              }`}
            >
              Yes
            </button>
